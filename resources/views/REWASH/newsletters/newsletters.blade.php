        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-newsletter_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Newsletters') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('REWASH.newsletters.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Newsletters') }}</h5>
                        @can('create newsletters')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('newsletters.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('newsletters_id') }}</th>
<th  >{{ __('newsletters_branch_id') }}</th>
<th  >{{ __('newsletters_email') }}</th>
<th  >{{ __('newsletters_created_at') }}</th>
<th  >{{ __('newsletters_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($newsletters as $key => $newsletter)
            <tr>
            <td>{{ $newsletter->firstItem + $key + 1 }}</td>
            
<td>{{ $newsletter->id }}</td>
<td>{{ $newsletter->branch_id }}</td>
<td>{{ $newsletter->email }}</td>
<td>{{ $newsletter->created_at }}</td>
<td>{{ $newsletter->updated_at }}</td>         <td>
           <a href="{{ route('newsletters.show', $newsletter->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $newsletter->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-newsletter_{{  $newsletter->id }}"
                                                    action="{{ route('newsletters.destroy', $newsletter->id) }}" method="POST"
                                                    style="display: none;">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="6"  >{{$newsletters->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
