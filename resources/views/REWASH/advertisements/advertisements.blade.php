...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Advertisements') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('REWASH.advertisements.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Advertisements') }}</h5>
                        @can('create advertisements')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('advertisements.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('advertisements_id') }}</th>
<th  >{{ __('advertisements_branch_id') }}</th>
<th  >{{ __('advertisements_title') }}</th>
<th  >{{ __('advertisements_image') }}</th>
<th  >{{ __('advertisements_is_active') }}</th>
<th  >{{ __('advertisements_created_at') }}</th>
<th  >{{ __('advertisements_updated_at') }}</th>
<th  >{{ __('advertisements_advertisement_type') }}</th>
<th  >{{ __('advertisements_advertisement_data') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
...
<td>
<img src="{{ $advertisement->image }}" higth="100" width="100">
 </td>
<td>{{ ($advertisement->is_active == 1) ? __("YES") : __("NO") }}</td>
<td>{{ $advertisement->created_at }}</td>
<td>{{ $advertisement->updated_at }}</td>
<td>{{ $advertisement->advertisement_type }}</td>
<td>{{ $advertisement->advertisement_data }}</td>         <td>
           <a href="{{ route('advertisements.show', $advertisement->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $advertisement->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-advertisement_{{  $advertisement->id }}"
                                                    action="{{ route('advertisements.destroy', $advertisement->id) }}" method="POST"
                                                    style="display: none;">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="10"  >{{$advertisements->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
