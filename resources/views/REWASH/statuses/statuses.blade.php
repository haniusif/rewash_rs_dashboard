        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-status_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Statuses') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('REWASH.statuses.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Statuses') }}</h5>
                        @can('create statuses')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('statuses.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('statuses_id') }}</th>
<th  >{{ __('statuses_branch_id') }}</th>
<th  >{{ __('statuses_status') }}</th>
<th  >{{ __('statuses_en_status') }}</th>
<th  >{{ __('statuses_created_at') }}</th>
<th  >{{ __('statuses_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($statuses as $key => $status)
            <tr>
            <td>{{ $status->firstItem + $key + 1 }}</td>
            
<td>{{ $status->id }}</td>
<td>{{ $status->branch_id }}</td>
<td>{{ $status->status }}</td>
<td>{{ $status->en_status }}</td>
<td>{{ $status->created_at }}</td>
<td>{{ $status->updated_at }}</td>         <td>
           <a href="{{ route('statuses.show', $status->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $status->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-status_{{  $status->id }}"
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="7"  >{{$statuses->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
