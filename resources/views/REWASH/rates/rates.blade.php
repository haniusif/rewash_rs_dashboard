        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-rate_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Rates') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('REWASH.rates.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Rates') }}</h5>
                        @can('create rates')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('rates.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('rates_id') }}</th>
<th  >{{ __('rates_branch_id') }}</th>
<th  >{{ __('rates_user_id') }}</th>
<th  >{{ __('rates_worker_id') }}</th>
<th  >{{ __('rates_appointment_id') }}</th>
<th  >{{ __('rates_appointment_schedule_id') }}</th>
<th  >{{ __('rates_rate_value') }}</th>
<th  >{{ __('rates_comments') }}</th>
<th  >{{ __('rates_created_at') }}</th>
<th  >{{ __('rates_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($rates as $key => $rate)
            <tr>
            <td>{{ $rate->firstItem + $key + 1 }}</td>
            
<td>{{ $rate->id }}</td>
<td>{{ $rate->branch_id }}</td>
<td>{{ $rate->user_id }}</td>
<td>{{ $rate->worker_id }}</td>
<td>{{ $rate->appointment_id }}</td>
<td>{{ $rate->appointment_schedule_id }}</td>
<td>{{ $rate->rate_value }}</td>
<td>{{ $rate->comments }}</td>
<td>{{ $rate->created_at }}</td>
<td>{{ $rate->updated_at }}</td>         <td>
           <a href="{{ route('rates.show', $rate->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $rate->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-rate_{{  $rate->id }}"
                                                    action="{{ route('rates.destroy', $rate->id) }}" method="POST"
                                                    style="display: none;">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="11"  >{{$rates->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
