        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-testimonial_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Testimonials') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('REWASH.testimonials.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Testimonials') }}</h5>
                        @can('create testimonials')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('testimonials.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('testimonials_id') }}</th>
<th  >{{ __('testimonials_branch_id') }}</th>
<th  >{{ __('testimonials_name') }}</th>
<th  >{{ __('testimonials_image') }}</th>
<th  >{{ __('testimonials_post') }}</th>
<th  >{{ __('testimonials_detail') }}</th>
<th  >{{ __('testimonials_created_at') }}</th>
<th  >{{ __('testimonials_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($testimonials as $key => $testimonial)
            <tr>
            <td>{{ $testimonial->firstItem + $key + 1 }}</td>
            
<td>{{ $testimonial->id }}</td>
<td>{{ $testimonial->branch_id }}</td>
<td>{{ $testimonial->name }}</td>
<td>
<img src="{{ $testimonial->image }}" higth="100" width="100">
 </td>
...
<td>{{ $testimonial->updated_at }}</td>         <td>
           <a href="{{ route('testimonials.show', $testimonial->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $testimonial->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-testimonial_{{  $testimonial->id }}"
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="9"  >{{$testimonials->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
