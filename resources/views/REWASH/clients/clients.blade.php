        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-client_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
     ...
     @include('REWASH.clients.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Clients') }}</h5>
                        @can('create clients')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('clients.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('clients_id') }}</th>
<th  >{{ __('clients_branch_id') }}</th>
<th  >{{ __('clients_image') }}</th>
<th  >{{ __('clients_created_at') }}</th>
<th  >{{ __('clients_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($clients as $key => $client)
            <tr>
            <td>{{ $client->firstItem + $key + 1 }}</td>
            
<td>{{ $client->id }}</td>
<td>{{ $client->branch_id }}</td>
<td>
<img src="{{ $client->image }}" higth="100" width="100">
 </td>
<td>{{ $client->created_at }}</td>
<td>{{ $client->updated_at }}</td>         <td>
           <a href="{{ route('clients.show', $client->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $client->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-client_{{  $client->id }}"
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="6"  >{{$clients->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
