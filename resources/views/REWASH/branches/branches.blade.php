        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-branch_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Branches') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('REWASH.branches.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Branches') }}</h5>
                        @can('create branches')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('branches.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('branches_id') }}</th>
<th  >{{ __('branches_name') }}</th>
<th  >{{ __('branches_en_name') }}</th>
<th  >{{ __('branches_is_active') }}</th>
<th  >{{ __('branches_created_at') }}</th>
<th  >{{ __('branches_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($branches as $key => $branch)
            <tr>
            <td>{{ $branch->firstItem + $key + 1 }}</td>
            
<td>{{ $branch->id }}</td>
<td>{{ $branch->name }}</td>
<td>{{ $branch->en_name }}</td>
<td>{{ ($branch->is_active == 1) ? __("YES") : __("NO") }}</td>
<td>{{ $branch->created_at }}</td>
<td>{{ $branch->updated_at }}</td>         <td>
           <a href="{{ route('branches.show', $branch->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $branch->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-branch_{{  $branch->id }}"
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="7"  >{{$branches->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
