        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-stock_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Stocks') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('REWASH.stocks.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Stocks') }}</h5>
                        @can('create stocks')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('stocks.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('stocks_id') }}</th>
<th  >{{ __('stocks_product_id') }}</th>
<th  >{{ __('stocks_quantity') }}</th>
<th  >{{ __('stocks_operation') }}</th>
<th  >{{ __('stocks_operation_date') }}</th>
<th  >{{ __('stocks_created_at') }}</th>
<th  >{{ __('stocks_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($stocks as $key => $stock)
            <tr>
            <td>{{ $stock->firstItem + $key + 1 }}</td>
...
<td>{{ $stock->updated_at }}</td>         <td>
           <a href="{{ route('stocks.show', $stock->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $stock->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-stock_{{  $stock->id }}"
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="8"  >{{$stocks->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
