        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-notification_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
     ...
     @include('REWASH.notifications.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        ...
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('notifications.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('notifications_id') }}</th>
<th  >{{ __('notifications_user_id') }}</th>
<th  >{{ __('notifications_is_read') }}</th>
<th  >{{ __('notifications_title') }}</th>
<th  >{{ __('notifications_notification') }}</th>
<th  >{{ __('notifications_en_title') }}</th>
<th  >{{ __('notifications_en_notification') }}</th>
<th  >{{ __('notifications_created_at') }}</th>
<th  >{{ __('notifications_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($notifications as $key => $notification)
            <tr>
            <td>{{ $notification->firstItem + $key + 1 }}</td>
...
<td>{{ $notification->en_title }}</td>
<td>{{ $notification->en_notification }}</td>
<td>{{ $notification->created_at }}</td>
<td>{{ $notification->updated_at }}</td>         <td>
           <a href="{{ route('notifications.show', $notification->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $notification->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-notification_{{  $notification->id }}"
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="10"  >{{$notifications->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
