        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-service_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Services') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('REWASH.services.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Services') }}</h5>
                        @can('create services')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('services.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('services_id') }}</th>
<th  >{{ __('services_branch_id') }}</th>
<th  >{{ __('services_icon') }}</th>
<th  >{{ __('services_name') }}</th>
<th  >{{ __('services_description') }}</th>
<th  >{{ __('services_created_at') }}</th>
<th  >{{ __('services_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($services as $key => $service)
            <tr>
            <td>{{ $service->firstItem + $key + 1 }}</td>
...
<td>{{ $service->updated_at }}</td>         <td>
           <a href="{{ route('services.show', $service->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $service->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-service_{{  $service->id }}"
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="8"  >{{$services->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
