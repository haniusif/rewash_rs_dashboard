        ...
        function showConfirmation(button) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var id = button.value;
                    document.getElementById('destroy-provider_'+id).submit();
...
<div class="row">
 <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Providers') }}
            </li>
        </ol>
    </div>
     @include('_partials.alerts')
     @include('REWASH.providers.search.search')
    <div class="col-12">
    <div class="card">
                    <div class="card-body">
                      <div class="card-title header-elements">
                        <h5 class="m-0 me-2">{{ __('Providers') }}</h5>
                        @can('create providers')
                        <div class="card-title-elements ms-auto">
                         <a  href="{{ route('providers.create') }}" class="btn btn-xs btn-primary waves-effect waves-light">
...
<th  >{{ __('providers_id') }}</th>
<th  >{{ __('providers_created_at') }}</th>
<th  >{{ __('providers_updated_at') }}</th> <th >{{ __('Action') }}</th>
    </tr>
     </thead>
          <tbody>
          @foreach ($providers as $key => $provider)
            <tr>
            <td>{{ $provider->firstItem + $key + 1 }}</td>
            
<td>{{ $provider->id }}</td>
<td>{{ $provider->created_at }}</td>
<td>{{ $provider->updated_at }}</td>         <td>
           <a href="{{ route('providers.show', $provider->id) }}"
                                                    class="btn btn-success mr-1 mb-1 btn-sm"><i
                                                        class="ti ti-eye"></i></a>
            <button value="{{  $provider->id }}" class="btn btn-danger mr-1 mb-1 btn-sm "  onclick="showConfirmation(this)"   ><i class="ti ti-trash"></i></button>
                                                <form id="destroy-provider_{{  $provider->id }}"
                                                    action="{{ route('providers.destroy', $provider->id) }}" method="POST"
                                                    style="display: none;">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
 ...
 @endforeach
        ...
        <tfoot>
       <tr><td      colspan="4"  >{{$providers->links("pagination::bootstrap-4")}} </td></tr>
        </tfoot>
</table>
                                    </div>
                                </div>
