...
class JohanTrans extends johan
{
    /**
     ...
     * @var string
     */
    protected $signature = 'johan:trans
    {--module : translate module table }
    {--refactor : refactor module en_name }
    ...
    public function handle()
    {
        ...
        if ($this->option('refactor')) {
            ...
            foreach ($Modules as $Module) {
                $headline = Str::headline($Module->en_name);
                $this->info(' headline ' . $headline);
                $transModules = Module::find($Module->id);
                $transModules->en_name = $headline;
                $transModules->update();
            }
        }
        if ($this->option('module')) {
            ...
            $Modules = Module::all();
            foreach ($Modules as $Module) {
                // $this->info( $Module->route .'  module was detected successfully');
                $Modulename = $Module->name;
                if ($this->confirm('translate ' . $Module->route . '  module ?', 'yes')) {
                    $Modulename = $this->translate($Module->en_name, $from_lan = 'en', $to_lan = 'ar');
                    $this->info('translated to ' . $Modulename);
                    $transModules = Module::find($Module->id);
                    $transModules->name = $Modulename;
                    $transModules->update();
                }
                $module_names[] = $Module->en_name;
                $module_list_to_show[] = [$this->tableStrlen($Module->id, 5), $this->tableStrlen($Module->parent_id, 5), $this->tableStrlen($Module->en_name, 30), $this->tableStrlen($Modulename, 50)];
            }
