...
class JohanSearch extends johan
{
    ...
    public function handle()
    {
        ...
        foreach ($columns as $column) {
            ...
            if (!in_array($COLUMN_NAME, $this->skipColumns) && !in_array($COLUMN_NAME, $not_add)) {
                ...
                if ($models) {
                    ...
                    $replace = ['{{models}}', '{{model}}', '{{colum_name}}', '{{name}}', '{{names}}', '{{main_colum_name}}', '{{pk}}'];
                    $items = [$models['prtn'], $models['srtn'], $COLUMN_NAME, $singular_model_name, $plural_model_name, $this->main_colum_name($models['prtn'], $conn), $this->main_pk($models['prtn'], $conn)];
                    $this->search_view_builder($stub_names, $replace, $items, $plural_model_name, $theamName, $searchDir);
        ...
        if ($withFunction) {
            ...
            $stub_names = [$plural_model_name => 'search_function_strat'];
            $replace = [];
            $items = [];
            ...
            foreach ($modelsForImport as $imodel) {
                ...
                $replace = ['{{name}}', '{{app_name}}', '{{names}}'];
                $plural_imodel = Str::plural($imodel);

                $items = [$imodel, $this->app_name, Str::lower($plural_imodel)];
                $this->search_function_builder($stub_names, $replace, $items, $plural_model_name, $theamName, $searchDir);
            }
            ...
            foreach ($columnsForFunction as $key => $colmn) {
                ...
                if ($key == (count($columnsForFunction) - 1)) {
                    ...
                    $replace = ['{{colum_name}}', '{{app_name}}', '{{names}}'];
                    $items = [$colmn['COLUMN_NAME'], $this->app_name, $plural_model_name];
                    $this->search_function_builder($stub_names, $replace, $items, $plural_model_name, $theamName, $searchDir);
                }
            ...
            foreach ($modelsForImport as $imodel) {
                $stub_names = [$plural_model_name => 'search_function_footer_return'];
                $replace = ['{{name}}', '{{app_name}}', '{{names}}'];
                $plural_imodel = Str::plural($imodel);
                ...
                if (end($modelsForImport) == $imodel) {

                    $stub_names = [$plural_model_name => 'theend'];
                    $replace = [];
                    $items = [];
                    $this->search_function_builder($stub_names, $replace, $items, $plural_model_name, $theamName, $searchDir);
        ...
        if ($controller) {
            ...
            $this->info('update controller ...');

            $Controllers_file = app_path("Http/Controllers/" . $this->app_name . "/" . $ucfirst_singular_model_name . "Controller.php");

            if (file_exists($Controllers_file)) {
                $file_get_contents = file_get_contents($Controllers_file);

                if (!$hideAll) {
                    $this->info('Controllers_file');
                    ...
                    if (file_exists($file_pointer)) {
                        ...
                        $function_file = file_get_contents($file_pointer);
                        //$this->get_string_between($string, $start, $end)
                        $get_string_between = $this->get_string_between($file_get_contents, "// pure", "return");
                        // print_r(trim($get_string_between));

                        $phrase = $Controllers_file;
                        $replace = [$get_string_between];
                        $items = [
                            "
                        ...
                        // print_r($create);
                        file_put_contents(app_path("Http/Controllers/" . $this->app_name . "/" . $ucfirst_singular_model_name . "Controller.php"), $create);

                    }

                } else {
                    $this->info('check file at : ' . $Controllers_file);

                }

            } else {
                $this->info('No such file or directory : ' . $Controllers_file);
            }
